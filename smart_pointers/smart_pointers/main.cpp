...
void test_make_allocate_shared() {
    {
        auto sp = std::make_shared<NeitherDefaultNorCopyConstructible>(
                NeitherDefaultNorCopyConstructible(0));
        std::weak_ptr<NeitherDefaultNorCopyConstructible> wp = sp;
        auto ssp = sp;
        sp.reset();
        assert(!wp.expired());
        ssp.reset();
    ...
    {
        auto sp = std::make_shared<Accountant>();
        assert(Accountant::constructed == 1);

        std::weak_ptr<Accountant> wp = sp;
        auto ssp = sp;
        sp.reset();
        assert(Accountant::constructed == 1);
...

    {
        ...
        auto sp = std::allocate_shared<Accountant>(alloc);
        int count = allocated;
        assert(allocated > 0);
        assert(allocate_called == 1);
        assert(Accountant::constructed == 1);
